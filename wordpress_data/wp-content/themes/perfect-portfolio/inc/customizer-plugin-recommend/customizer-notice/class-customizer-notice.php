<?php
/**
 * Perfect_Portfolio Customizer Notification System.
 *
 ...
 */
class Perfect_Portfolio_Customizer_Notice {
	...
	/**
	 * The single instance of Perfect_Portfolio_Customizer_Notice
	 *
	 * @var Perfect_Portfolio_Customizer_Notice $instance The Perfect_Portfolio_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Perfect_Portfolio_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Perfect_Portfolio_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Perfect_Portfolio_Customizer_Notice ) ) {
			self::$instance = new Perfect_Portfolio_Customizer_Notice;
	...
	public function setup_config() {
		...
		global $perfect_portfolio_recommended_plugins;
		...
		$perfect_portfolio_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$perfect_portfolio_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'perfect-portfolio-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', array(), PERFECT_PORTFOLIO_THEME_VERSION );
		...
		wp_enqueue_script( 'perfect-portfolio-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'customize-controls' ), PERFECT_PORTFOLIO_THEME_VERSION );
		wp_localize_script(
			'perfect-portfolio-customizer-notice', 'customizer_notice_data', array(
				'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
				'template_directory'       => get_template_directory_uri(),
				'base_path'                => admin_url(),
				'activating_string'        => __( 'Activating', 'perfect-portfolio' ),
			)
		);
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the Perfect_Portfolio_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Perfect_Portfolio_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Perfect_Portfolio_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$show_recommended_plugins = get_option( 'perfect_portfolio_show_recommended_plugins' );
			...
			update_option( 'perfect_portfolio_show_recommended_plugins', $show_recommended_plugins );
		}
