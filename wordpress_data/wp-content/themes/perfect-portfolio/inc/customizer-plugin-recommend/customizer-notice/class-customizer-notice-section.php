<?php
/**
 * Perfect_Portfolio Customizer Notification Section Class.
 *
 * @package Perfect_Portfolio
 */

/**
 * Perfect_Portfolio_Customizer_Notice_Section class
 */
class Perfect_Portfolio_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		...
		foreach ( $perfect_portfolio_recommended_plugins as $slug => $plugin_opt ) {
			...
			$perfect_portfolio_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$perfect_portfolio_recommended_plugin['class'] = 'active';
			} else {
				$perfect_portfolio_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$perfect_portfolio_recommended_plugin['button_class'] = 'install-now button';
					$perfect_portfolio_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$perfect_portfolio_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$perfect_portfolio_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$perfect_portfolio_recommended_plugin['button_class'] = 'deactivate-now button';
					$perfect_portfolio_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$perfect_portfolio_recommended_plugin['id'] = $slug;
			$perfect_portfolio_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$perfect_portfolio_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$perfect_portfolio_recommended_plugin['description'] = $info->short_description;
			}

			$perfect_portfolio_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $perfect_portfolio_recommended_plugin;
