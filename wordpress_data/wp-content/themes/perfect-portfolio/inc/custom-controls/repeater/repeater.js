/*jshint -W065 */
var perfectportfolioRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['perfect-portfolio-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
	...
	addRow: function( data ){
		...
		if ( template ) {
			...
			newRow = new perfectportfolioRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'perfect_portfolio_get_fontawesome_ajax',
					perfect_portfolio_customize_nonce: perfect_portfolio_customize.nonce
				},
				beforeSend: function(){
