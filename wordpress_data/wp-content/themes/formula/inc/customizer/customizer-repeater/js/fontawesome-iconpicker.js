(function ($) {
    ...
    wp.formula_customizerRepeater = {
        ...
        init: function () {
            $('.iconpicker-items>i').on('click', function () {
                ...
                formula_customizer_repeater_refresh_social_icons(th);
                return false;
            });
        },
    ...
    $(document).ready(function () {
        wp.formula_customizerRepeater.init();

        $('.iconpicker-search').on('keyup', function () {
            wp.formula_customizerRepeater.search($(this));
        });

        $('.icp-auto').on('click', function () {
                wp.formula_customizerRepeater.iconPickerToggle($(this));
        });
